...
//            distances.push_back(matches01[i][1].distance);
        ...
        std::cout << "matches01 distances min/median/max: " << distances[0] << "/" << distances[distances.size() / 2] << "/" << distances[distances.size()] << std::endl;
    }
    for (int k = 0; k < 2; ++k) {
...
//            distances.push_back(matches10[i][1].distance);
        ...
        std::sort(distances.begin(), distances.end());
        std::cout << "matches10 distances min/median/max: " << distances[0] << "/" << distances[distances.size() / 2] << "/" << distances[distances.size()] << std::endl;
    }
    for (int k = 0; k < 2; ++k) {
        std::vector<cv::DMatch> matchesK;
...
//        // TODO добавьте left-right check, т.е. проверку правда ли если для точки А самой похожей оказалась точка Б, то вероятно при обратном сопоставлении и у точки Б - ближайшей является точка А
        ...
        // Я оставлю все методы(не зря же я их делал), но по сути достаточно K-ratio
...
//        // TODO добавьте left-right check, т.е. проверку правда ли если для точки А самой похожей оказалась точка Б, то вероятно при обратном сопоставлении и у точки Б - ближайшей является точка А
    ...
    img1to0 = cv::Mat(img0.rows, img0.cols * 1.55, CV_8UC3); // давайте теперь вторую картинку нарисуем не просто в пространстве первой картинки - но поверх нее!
...
//    cv::Mat img1_alternative = cv::imread(path + "box1_nesquik.png");
//    rassert(!img0.empty() && !img1.empty() && !img1_alternative.empty(), 2389827851080019);
    rassert(!img0.empty(), 489264926897452)
    rassert(!img1.empty(), 489264926897452)
    rassert(img0.type() == CV_8UC3, 2389827851080020);
    rassert(img1.type() == CV_8UC3, 2389827851080021);
//    rassert(img1_alternative.type() == CV_8UC3, 2389827851080023);
