...
cv::Mat convertBGRToGray(cv::Mat img) {
    ...
    for (int j = 0; j < height; ++j) {
        for (int i = 0; i < width; ++i) {
            ...
            grayIntensity =  0.299 * (float) red + 0.587 * (float) green + 0.114 * (float) blue;
...
cv::Mat convertDXYToGradientLength(cv::Mat img) {
    ...
    // точнее - его длину, ведь градиент - это вектор (двумерный, ведь у него две компоненты), а у вектора всегда есть длина - sqrt(x^2+y^2)
