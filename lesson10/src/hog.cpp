...
double fullStrength(HoG hog){
    double fullStrength = 0.0;
...
std::ostream &operator<<(std::ostream &os, const HoG &hog) {
    ...
    double fullStrengt = fullStrength(hog);

    os << "HoG[";
    for (int bin = 0; bin < NBINS; ++bin) {
        double angleInDegrees = bin * 45 + 22.5;
        int percentage = hog[bin] / fullStrengt * 100;
...
double distance(HoG a, HoG b) {
    ...
    double fullStrengthA = fullStrength(a);
    double fullStrengthB = fullStrength(b);

    double res = 0.0;
    for(int bin = 0; bin < NBINS; bin++){
        int percentageA = a[bin] / fullStrengthA * 100;
        int percentageB = b[bin] / fullStrengthB * 100;
        res += pow2(percentageA - percentageB);
    }
    res = sqrt(res);
    return res;
}
